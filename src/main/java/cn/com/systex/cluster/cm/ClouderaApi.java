...
public class ClouderaApi {
    ...
    static {
        apiRoot =
                new ClouderaManagerClientBuilder().withHost("10.201.26.111")
                    .withUsernamePassword("admin", "admin").build().getRootV10();
        clusters = apiRoot.getClustersResource().readClusters(DataView.SUMMARY);
        resource = apiRoot.getClustersResource().getServicesResource(clusters.get(0).getName());
    }
    ...
    public static void main(String[] args) {
        ...
        List<ApiHealthCheck> list = as.getHealthChecks();
        for(ApiHealthCheck check:list){
            System.out.println(check.getName()+","+check.getSummary());
        }
